...
class Solution {
    func maxProfit(_ prices: [Int]) -> Int {
        ...
        For example, in [7, 1, 5, 3, 6, 4] case, if we skip peaki and valleyj trying to obtain more profit by considering points with more difference in heights, the net profit obtained will always be lesser than the one obtained by including them, since C will always be lesser than A+B.
...
class TestCase: XCTestCase {
    ...
    func testRegular() {
        XCTAssertEqual(sol.maxProfit([7,1,5,3,6,4]), 7)
    }

    func testIncreasingArray() {
        XCTAssertEqual(sol.maxProfit([1,2,3,4,5]), 4)
    }

    func testDecreasingArray() {
        XCTAssertEqual(sol.maxProfit([7,6,4,3,1]), 0)
    }

    func testOneElement() {
        XCTAssertEqual(sol.maxProfit([5]), 0)
    }
}
